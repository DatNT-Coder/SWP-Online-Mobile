    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if (message.equals("addPost")) {
                ...
                int postID = daoPost.addPost(new BlogPost(title, brief_info, fileName, details, updatedDate, getPostCategories_id, id, true, 0, " "));
                if (postID > 0) {
                    request.setAttribute("message", "<span style=\"color:green;\">Tạo bài đăng thành công, hãy hiển thị bài đăng với khách hàng</span>");
                    Map<String, Object> postDetail = daoPost.getPostbyPostID(postID);
                    request.setAttribute("postDetails", postDetail);
                    request.getRequestDispatcher("/Marketing_viewPost.jsp").forward(request, response);
                } else {
                    request.setAttribute("message", "<span style=\"color:red;\">Tạo bài đăng không thành công!!!</span>");
                    request.getRequestDispatcher("/Marketing_viewPost.jsp").forward(request, response);
                }
