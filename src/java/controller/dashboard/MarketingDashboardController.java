...
public class MarketingDashboardController extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      int totalFeedback = fDAO.countProductFeedback();

      // Get new customers by date range
      List<User> newCustomers = uDAO.searchUsersByRegistrationDate(fromDate, toDate);

      // Prepare customer trend data for last 7 days
      Map<Date, Integer> customerTrend = new HashMap<>();
      Date currentDate = new Date(fromDate.getTime());

      while (currentDate.before(toDate) || currentDate.equals(toDate)) {
         Date nextDate = new Date(currentDate.getTime() + 24 * 60 * 60 * 1000);
         int count = uDAO.countUsersRegisteredBetween(currentDate, nextDate);
         customerTrend.put(currentDate, count);
